...
async def test_chat_completions_validation_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(proxy_router, "validate_proxy_api_key", lambda _key, **kw: (False, "", False))
...
async def test_logging_flush_loop_and_reload_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("ai_proxy.app.build_rate_limiters", lambda _providers: None)
    monkeypatch.setattr("ai_proxy.config.loader.reload_config", lambda _path, **kw: SimpleNamespace(providers={}))
